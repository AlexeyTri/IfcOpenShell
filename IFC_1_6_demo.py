...
table_pipes = list()
table_pipes.append(["Name", "Deskription", "Parent_networks", "Diametet", "Thickness", "Geometry"])
...
# начало работ сетей
xml_pipe_netwoks_collection = xml_file.find(ns_xmlns + "PipeNetworks")
for xml_pipe_netwoks in xml_pipe_netwoks_collection:
    network_name = xml_pipe_netwoks.attrib["name"]
    table_pipe_networks.append([network_name, xml_pipe_netwoks.attrib["desc"]])
    #колодцы
    ...
    """
    xml_strucs_cillection = xml_pipe_netwoks.find(ns_xmlns + "Structs").findall(ns_xmlns + "Struct")
    for xml_struct in xml_strucs_cillection:
        ...
        xml_pipes_collection = xml_pipe_netwoks.find(ns_xmlns + "Pipes").findall(ns_xmlns + "Pipe")
...
mguu_cource_tools.write_to_file("ark_surface", table_surfaces)
