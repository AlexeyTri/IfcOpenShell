...
# Перебор файлов в папке, путь вставить нужное
...
file_ifc_list_temp = list()

all_files_temp = os.listdir(root_path_os)
for one_file in all_files_temp:
    if one_file.endswith(".ifc"):
        file_ifc_list_temp.append(os.path.join(root_path_os, one_file))

#Работаем с файлами
out_dict = dict()
for one_path in file_ifc_list_temp:
    print(one_path)
